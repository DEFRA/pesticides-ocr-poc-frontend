...
// Recover the intended HTTP status from errors thrown by the @defra/hapi-oidc-auth
// layer. Those are plain Error objects carrying a numeric `statusCode` (e.g.
// 401/422); Hapi boomifies a non-Boom throw to 500 without reading that property,
// so the intended status would otherwise be lost. Only override when Hapi
// defaulted to 500 and the error advertised a valid client/server error status.
